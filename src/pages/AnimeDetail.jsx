...
export default function AnimeDetail() {
      ...
      const fetchTrailer = async () => {
        try {
          const searchRes = await fetch(
            `https://api.themoviedb.org/3/search/tv?api_key=1fa448de74c5dac88e0d31d99c7e916d&query=${encodeURIComponent(a.title)}`,
          );
          const searchData = await searchRes.json();
          if (searchData.results?.[0]?.id) {
            const vidRes = await fetch(
              `https://api.themoviedb.org/3/tv/${searchData.results[0].id}/videos?api_key=1fa448de74c5dac88e0d31d99c7e916d`,
            );
      ...
      try {
        ...
        if (tmdbId) {
          const recRes = await fetch(
            `https://api.themoviedb.org/3/tv/${tmdbId}/recommendations?api_key=1fa448de74c5dac88e0d31d99c7e916d&language=en-US&page=1`,
          );
          const recData = await recRes.json();
          if (recData.results?.length > 0) {
            setTmdbRecs(
              recData.results.slice(0, 6).map((r) => ({
                id: r.id,
  ...
  const recommended =
    tmdbRecs.length > 0 ? tmdbRecs : a.recommended || animeDetail.recommended;
  ...
  return (
    <div className="anime-detail">
      ...
      {/* Hero */}
      <div
        className="anime-detail__hero"
        style={{ backgroundImage: `url("${a.hero}")` }}
      >
        ...
        <section className="anime-section">
          <div className="section-heading">
            ...
            <div className="anime-nav-arrows">
              <button className="anime-nav-btn">
                <ChevronLeft size={18} />
              </button>
              <button className="anime-nav-btn">
          ...
          <div className="episodes-grid">
            {episodes_list.map((ep, i) => (
              <div
                key={ep.id}
                className={`episode-card ${i === 0 ? "episode-card--current" : ""}`}
                onClick={() => navigate(`/watch/${a.id}`)}
              >
                <div className="episode-card__thumb">
                  ...
                  {i === 0 && (
                    <span className="episode-card__now-playing label">
                      Now Playing
                    </span>
                  )}
                </div>
                <div className="episode-card__info">
                  <span className="episode-card__number label">
                    {ep.number}
                  </span>
        ...
        <section className="anime-trailer-section">
          <div className="anime-trailer-player">
            {!playing ? (
              ...
              >
                <img
                  src={stills && stills[0] ? stills[0] : a.hero}
                  alt="Trailer"
          ...
          <div className="anime-features">
            ...
            <p className="anime-features__desc">
              Experience the adrenaline-fueled world of Night City in this
              official series trailer. Witness the stunning animation by Studio
              Trigger and the pulse-pounding soundtrack.
            </p>
